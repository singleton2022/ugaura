...
def generate_cushion_csv(target_date=None, cushion_map=None, drive_dir=None):
    ...
    rows = get_turf_race_rows(db_path, year_val, month_day_val)
    if not rows:
        print(f"{target_date} \u306e\u829d\u30ec\u30fc\u30b9\u306e\u30ec\u30b3\u30fc\u30c9\u306f\u898b\u3064\u304b\u308a\u307e\u305b\u3093\u3067\u3057\u305f\u3002")
        return False

    if cushion_map is None:
        ...
        for code in course_codes:
            ...
            val = input(f"\u7af6\u99ac\u5834\u30b3\u30fc\u30c9 {code_str} ({course_name}) \u306e\u30af\u30c3\u30b7\u30e7\u30f3\u5024: ").strip()
            cushion_map[code] = val

    output_file = f"\u30af\u30c3\u30b7\u30e7\u30f3\u5024{target_date}.csv"
    ...
    with open(output_path, mode='w', encoding='cp932', newline='') as f:
        ...
        for row in rows:
            ...
            cushion_val = cushion_map.get(cc, "")
            writer.writerow([horse_id_18, cushion_val])
            generated_rows.append((horse_id_18, cushion_val))

    print(f"\nCSV\u30d5\u30a1\u30a4\u30eb\u3092\u4f5c\u6210\u3057\u307e\u3057\u305f: {output_path}")
    print(f"\u7dcf\u884c\u6570: {len(rows)} (\u829d\u30ec\u30fc\u30b9\u306e\u307f)")

    local_yearly = os.path.join(os.path.dirname(__file__), f"\u30af\u30c3\u30b7\u30e7\u30f3\u5024{year_val}.csv")
    ...
    if drive_dir and os.path.exists(drive_dir):
        drive_yearly = os.path.join(drive_dir, f"\u30af\u30c3\u30b7\u30e7\u30f3\u5024{year_val}.csv")
